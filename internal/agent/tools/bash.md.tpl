...
2. Security Check: {{ if .BypassSecurity }}Security bypass is enabled - all commands are allowed without restriction{{ else }}Banned commands ({{ .BannedCommands }}) return error - explain to user. Safe read-only commands execute without prompts{{ end }}
